...
def apply_primitive(procedure, args, env):
    ...
    python_args = []
    scheme_args = args
    while scheme_args != nil:
        python_args.append(scheme_args.first)
        scheme_args = scheme_args.second
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("Arguments not equal")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        Frame.define(env, target, scheme_eval(vals[1], env))
        return vals[0]
    elif isinstance(target, Pair):
        if scheme_symbolp(target[0]):
            Frame.define(env, target[0], do_lambda_form(Pair(target.second, vals.second), env))
            return target[0]
...
def do_let_form(vals, env):
    ...
    for el in bindings:
        names = Pair(el[0], names)
        values = Pair(scheme_eval(el[1], env), values)
...
def do_and_form(vals, env):
    ...
    incrementer, ln=0, len(vals) - 1
    for i in list(vals):
        if incrementer == ln:
            return i
        incrementer+=1
        if not scheme_eval(i, env):
            return False
    return True
...
def do_or_form(vals, env):
    ...
    incrementer=0
    for i in list(vals):
        if incrementer == len(vals):
            return i
        incrementer+=1
        if scheme_eval(i, env):
            return quote(i)
    return False
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            if len(clause.second) < 1:
                return quote(scheme_eval(clause[0], env))
            else:
                return clause[1]
    return okay
...
def check_formals(formals):
    ...
    lst = []
    for i in formals:
       if scheme_symbolp(i) != True or (i in lst):
